...
async function main() {
    ...
    const addressArg = process.env.ArgAddress;  // Get the referenced contract address from environment variables

    // Check if the contract name was provided
    if (!contractName || !addressArg) {
        console.error("Error: No contract name or addressArg provided.");
        console.error("Usage: Contract=<ContractName> ArgAddress=<address> npx hardhat run scripts/deploy.js --network localhost");
        process.exit(1);
    }
    ...
    try {
        ...
        // Step 4: Deploy the contract with input argument set to address
        const contractInstance = await Contract.deploy(addressArg);
